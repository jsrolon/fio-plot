...
def run_raw_command(command, env=None):
    ...
    if result.returncode != 0:
        logger.error(result.stderr.read(100000))
        if result.returncode != 127 and "is_backend" not in result.stderr.read(10000): # ridiculous workaround for spdk fio plugin
            sys.exit(1)
